...
def test_r31do_ray_heuristic_failure_falls_back_to_random(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    run_dir = tmp_path / "run_ray_heuristic_fail"
    ...
    with ExperimentDB(db_path, engine="sqlite") as db:
        run_id = db.create_run(problem_hash="ph_r31do", spec={}, meta={"source": "test"})
        coord._run_ray(
            args,
