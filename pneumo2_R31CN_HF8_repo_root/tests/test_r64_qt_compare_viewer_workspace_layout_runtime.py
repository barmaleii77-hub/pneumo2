...
def test_compare_viewer_view_menu_exposes_workspace_presets_and_dock_families(monkeypatch) -> None:
    ...
    try:
        ...
        assert "\u041f\u0430\u043d\u0435\u043b\u0438 5/13" in viewer.lbl_status_layout.text()
        ...
        assert viewer.dockWidgetArea(viewer.dock_compare_contract) == QtCore.Qt.LeftDockWidgetArea
        ...
        saved_state = viewer.saveState()
        viewer.dock_compare_contract.hide()
        app.processEvents()
        assert not viewer.dock_compare_contract.isVisible()
        assert viewer.restoreState(saved_state) is True
        app.processEvents()
        assert viewer.dock_compare_contract.isVisible()
        ...
        assert viewer.dock_compare_contract.isFloating() is False
