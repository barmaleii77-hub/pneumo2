...
def test_v38_kb_commit_ready_import_layer_exists_and_loads_contracts() -> None:
    required_files = {
        "README.md",
...
def test_project_sources_index_and_import_notes_register_v13_addendum() -> None:
    ...
    assert "current successor" in imports_readme
...
def test_canon_and_parity_summary_reference_v13_ring_editor_layer() -> None:
    ...
    assert "## У. V38 GitHub KB commit-ready successor layer" in canon_18
    assert "## Ф. V37 GitHub KB supplement и TZ/spec connector" in canon_18
...
def test_v12_design_recovery_layer_and_lineage_inventory_are_registered() -> None:
    ...
    assert "commit-ready successor package" in lineage_md
    ...
    assert any(
        ...
        and item["status"] == "predecessor_successor_kb_supplement"
