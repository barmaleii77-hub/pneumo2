...
def _make_fake_ray_module() -> ModuleType:
    ...
    def cluster_resources() -> Dict[str, float]:
        # GPU=0 keeps proposer actor pool disabled, forcing local proposer branch.
...
def _base_args() -> SimpleNamespace:
    return SimpleNamespace(
        ...
        botorch_maxiter=200,
        heuristic_pool_size=530,
        heuristic_explore=0.34,
    )
...
def _seed_one_done_trial(db: ExperimentDB, run_id: str, problem_hash: str, core: _DummyCore) -> None:
    ...
    res_done = db.reserve_trial(
        ...
        param_hash="portfolio_ray_fallback_seed_done",
        x_u=list(x_done),
        params=params_done,
    )
    db.mark_done(
        ...
        y=[1.3, 2.3],
        g=[-0.09],
        metrics={"obj1": 1.3, "obj2": 2.3, "penalty_total": -0.09},
    )


def test_r31dt_ray_portfolio_qnehvi_failure_falls_back_to_heuristic_with_random_blend(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    def fake_propose_qnehvi(**kwargs):  # noqa: ARG001
        q_calls["n"] += 1
        raise RuntimeError("forced qnehvi failure for portfolio ray")

    def fake_propose_heuristic(**kwargs):
        h_calls["n"] += 1
        return SimpleNamespace(
            X=np.asarray([[0.28, 0.68]], dtype=float),
            meta={"method": "heuristic_after_portfolio_qfail_ray", "source": "r31dt_test"},
        )
    ...
    def fake_propose_random(*, d, q, seed=0):  # noqa: ARG001
        ...
        q_i = max(1, int(q))
        d_i = max(1, int(d))
        X = np.full((q_i, d_i), 0.93, dtype=float)
    ...
    run_dir = tmp_path / "run_ray_portfolio_qfail_to_heur"
    ...
    with ExperimentDB(db_path, engine="sqlite") as db:
        run_id = db.create_run(problem_hash="ph_r31dt_qh", spec={}, meta={"source": "test"})
        _seed_one_done_trial(db, run_id, "ph_r31dt_qh", core)
        coord._run_ray(
            args,
            core_local=core,
            db=db,
    ...
    assert meta["method"] == "heuristic_after_portfolio_qfail_ray"
    assert (run_dir / "export" / "trials.csv").exists()


def test_r31dt_ray_portfolio_qnehvi_and_heuristic_failure_fall_back_to_random_with_extra_blend(
    tmp_path: Path,
    monkeypatch,
) -> None:
    fake_ray = _make_fake_ray_module()
    ...
    def fake_propose_heuristic(**kwargs):  # noqa: ARG001
        h_calls["n"] += 1
        raise ValueError("forced heuristic failure for portfolio ray")

    def fake_propose_random(*, d, q, seed=0):  # noqa: ARG001
        rand_calls["n"] += 1
        q_i = max(1, int(q))
        d_i = max(1, int(d))
        X = np.full((q_i, d_i), 0.94, dtype=float)
        return SimpleNamespace(X=X)
    ...
    db_path = tmp_path / "experiments.sqlite"
    run_dir = tmp_path / "run_ray_portfolio_qh_fail_to_random"
    run_dir.mkdir(parents=True, exist_ok=True)
    ...
    with ExperimentDB(db_path, engine="sqlite") as db:
        run_id = db.create_run(problem_hash="ph_r31dt_qhr", spec={}, meta={"source": "test"})
        _seed_one_done_trial(db, run_id, "ph_r31dt_qhr", core)
        coord._run_ray(
            args,
            core_local=core,
            db=db,
