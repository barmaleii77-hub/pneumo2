...
def _make_fake_ray_module() -> ModuleType:
    ...
    def cluster_resources() -> Dict[str, float]:
        # Two GPUs allow two proposer actors.
...
def test_r31ed_ray_pool_partial_failure_preserves_surviving_actor_for_next_cycle(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    def fake_propose_qnehvi(**kwargs):  # noqa: ARG001
        q_calls["n"] += 1
        if q_calls["n"] == 1:
            raise RuntimeError("forced first actor failure")
        return SimpleNamespace(
            X=np.asarray([[0.26, 0.66]], dtype=float),
            meta={"method": "qnehvi_pool_actor_survival", "source": "r31ed_test"},
        )

    def fake_propose_heuristic(**kwargs):  # noqa: ARG001
        h_calls["n"] += 1
        raise AssertionError("heuristic must not be used in this scenario")

    def fake_propose_random(*, d, q, seed=0):  # noqa: ARG001
        rand_calls["n"] += 1
        q_i = max(1, int(q))
        d_i = max(1, int(d))
        X = np.full((q_i, d_i), 0.79, dtype=float)
    ...
    run_dir = tmp_path / "run_ray_pool_actor_survival"
    ...
    args = SimpleNamespace(
        ...
        botorch_maxiter=200,
        heuristic_pool_size=680,
        heuristic_explore=0.36,
    )

    core = _DummyCore(dim=2)
    with ExperimentDB(db_path, engine="sqlite") as db:
        run_id = db.create_run(problem_hash="ph_r31ed", spec={}, meta={"source": "test"})
        seed_x = [0.13, 0.83]
        seed_params = core.u_to_params(seed_x)
        seed_res = db.reserve_trial(
            run_id=run_id,
            problem_hash="ph_r31ed",
            param_hash="pool_actor_survival_seed_done",
            x_u=list(seed_x),
            params=seed_params,
        )
        db.mark_done(
    ...
    # First fill: 2 actor calls (1 fail + 1 success).
    # Second fill: surviving actor stays in pool and contributes one more call.
    assert q_calls["n"] == 3
    assert h_calls["n"] == 0
    ...
    if str(meta.get("effective_mode") or "") == "qnehvi":
        assert meta["method"] == "qnehvi_pool_actor_survival"
        # Actor key indicates second cycle still used proposer pool (not local fallback).
        assert str(meta.get("actor") or "").startswith("prop_")
        assert "pool_fallback_error" not in meta
    else:
        # Duplicate-safe scheduling may trigger a rescue candidate near the end of budget.
        assert str(meta.get("effective_mode") or "") == "random"
