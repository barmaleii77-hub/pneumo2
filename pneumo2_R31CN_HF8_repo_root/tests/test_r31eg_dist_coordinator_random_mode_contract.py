...
def test_r31eg_dask_random_mode_reports_forced_random_reason(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    def fake_propose_qnehvi(**kwargs):  # noqa: ARG001
        q_calls["n"] += 1
        raise AssertionError("qnehvi must not be used in random mode")

    def fake_propose_heuristic(**kwargs):  # noqa: ARG001
        h_calls["n"] += 1
        raise AssertionError("heuristic must not be used in random mode")

    def fake_propose_random(*, d, q, seed=0):  # noqa: ARG001
        rand_calls["n"] += 1
        q_i = max(1, int(q))
        d_i = max(1, int(d))
        X = np.full((q_i, d_i), 0.45, dtype=float)
    ...
    run_dir = tmp_path / "run_dask_forced_random"
    ...
    with ExperimentDB(db_path, engine="sqlite") as db:
        run_id = db.create_run(problem_hash="ph_r31eg_dask", spec={}, meta={"source": "test"})
        coord._run_dask(
            args,
    ...
    assert meta["requested_mode"] == "random"
    assert meta["effective_mode"] == "random"
    assert meta["reason"] == "forced_random"


def test_r31eg_ray_random_mode_reports_forced_random_reason(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    def fake_propose_random(*, d, q, seed=0):  # noqa: ARG001
        ...
        q_i = max(1, int(q))
        d_i = max(1, int(d))
        X = np.full((q_i, d_i), 0.46, dtype=float)
        return SimpleNamespace(X=X)
    ...
    db_path = tmp_path / "experiments.sqlite"
    run_dir = tmp_path / "run_ray_forced_random"
    run_dir.mkdir(parents=True, exist_ok=True)
    ...
    with ExperimentDB(db_path, engine="sqlite") as db:
        run_id = db.create_run(problem_hash="ph_r31eg_ray", spec={}, meta={"source": "test"})
        coord._run_ray(
            args,
            core_local=_DummyCore(dim=2),
            db=db,
