...
def _make_fake_ray_module() -> ModuleType:
    ...
    def cluster_resources() -> Dict[str, float]:
        # GPU>0 enables proposer actor pool path. Use 2 GPUs to allow 2 pool actors.
...
def test_r31ec_ray_portfolio_pool_partial_failure_keeps_pool_success_and_blend_without_local_fallback(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    def fake_propose_qnehvi(**kwargs):  # noqa: ARG001
        q_calls["n"] += 1
        if q_calls["n"] == 1:
            raise RuntimeError("forced first portfolio pool actor failure")
        return SimpleNamespace(
            X=np.asarray([[0.34, 0.64]], dtype=float),
            meta={"method": "portfolio_pool_partial_success", "source": "r31ec_test"},
        )

    def fake_propose_heuristic(**kwargs):  # noqa: ARG001
        h_calls["n"] += 1
        raise AssertionError("heuristic must not be used when pool already produced candidates")

    def fake_propose_random(*, d, q, seed=0):  # noqa: ARG001
        rand_calls["n"] += 1
        q_i = max(1, int(q))
        d_i = max(1, int(d))
        X = np.full((q_i, d_i), 0.88, dtype=float)
    ...
    run_dir = tmp_path / "run_ray_portfolio_pool_partial_failure"
    ...
    args = SimpleNamespace(
        ...
        botorch_maxiter=200,
        heuristic_pool_size=650,
        heuristic_explore=0.35,
    )

    core = _DummyCore(dim=2)
    with ExperimentDB(db_path, engine="sqlite") as db:
        run_id = db.create_run(problem_hash="ph_r31ec", spec={}, meta={"source": "test"})
        seed_x = [0.12, 0.82]
        seed_params = core.u_to_params(seed_x)
        seed_res = db.reserve_trial(
            run_id=run_id,
            problem_hash="ph_r31ec",
            param_hash="portfolio_pool_partial_seed_done",
            x_u=list(seed_x),
            params=seed_params,
        )
        db.mark_done(
    ...
    # Two pool actors only; no extra local qNEHVI fallback call.
    assert q_calls["n"] == 2
    assert h_calls["n"] == 0
    # Portfolio blend is added from pool-success path.
    ...
    assert meta["method"] == "portfolio_pool_partial_success"
