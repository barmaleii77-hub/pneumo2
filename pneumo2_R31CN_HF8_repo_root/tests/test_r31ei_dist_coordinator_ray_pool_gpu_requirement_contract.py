...
def test_r31ei_ray_pool_is_disabled_when_gpus_per_proposer_exceeds_available_gpu_budget(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    def fake_propose_qnehvi(**kwargs):  # noqa: ARG001
        ...
        return SimpleNamespace(
            X=np.asarray([[0.39, 0.69]], dtype=float),
            meta={"method": "qnehvi_local_when_pool_disabled", "source": "r31ei_test"},
        )
    ...
    run_dir = tmp_path / "run_ray_pool_disabled_by_gpu_requirement"
    ...
    args = SimpleNamespace(
        ...
        ray_num_proposers=4,
        ray_gpus_per_proposer=2.0,  # exceeds available 1 GPU
        proposer_buffer=1,
        max_inflight=0,
        seed=0,
        budget=2,
        ...
        botorch_maxiter=200,
        heuristic_pool_size=760,
        heuristic_explore=0.37,
    )

    core = _DummyCore(dim=2)
    with ExperimentDB(db_path, engine="sqlite") as db:
        run_id = db.create_run(problem_hash="ph_r31ei", spec={}, meta={"source": "test"})
        seed_x = [0.17, 0.87]
        seed_params = core.u_to_params(seed_x)
        seed_res = db.reserve_trial(
            run_id=run_id,
            problem_hash="ph_r31ei",
            param_hash="qnehvi_pool_disabled_seed_done",
            x_u=list(seed_x),
            params=seed_params,
        )
        db.mark_done(
    ...
    # Local qNEHVI path is used; no pool actor metadata.
    assert q_calls["n"] >= 1
    assert h_calls["n"] == 0
    ...
    assert meta["method"] == "qnehvi_local_when_pool_disabled"
    assert "actor" not in meta
