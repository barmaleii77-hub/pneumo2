...
def _make_fake_ray_module() -> ModuleType:
    ...
    def cluster_resources() -> Dict[str, float]:
        # GPU>0 triggers proposer actor pool path.
...
def test_r31dp_ray_proposer_pool_failure_degrades_to_local_fallback(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    def fake_propose_qnehvi(**kwargs):  # noqa: ARG001
        q_calls["n"] += 1
        raise RuntimeError("forced qnehvi failure in pool/local")

    def fake_propose_heuristic(**kwargs):
        h_calls["n"] += 1
        return SimpleNamespace(
            X=np.asarray([[0.26, 0.56]], dtype=float),
            meta={"method": "heuristic_after_pool_fail", "source": "r31dp_test"},
        )
    ...
    run_dir = tmp_path / "run_ray_pool_fail_fallback"
    ...
    args = SimpleNamespace(
        ...
        botorch_maxiter=200,
        heuristic_pool_size=555,
        heuristic_explore=0.36,
    )

    core = _DummyCore(dim=2)
    with ExperimentDB(db_path, engine="sqlite") as db:
        run_id = db.create_run(problem_hash="ph_r31dp", spec={}, meta={"source": "test"})
        seed_x = [0.12, 0.88]
        seed_params = core.u_to_params(seed_x)
        seed_res = db.reserve_trial(
            run_id=run_id,
            problem_hash="ph_r31dp",
            param_hash="pool_fail_seed_done",
            x_u=list(seed_x),
            params=seed_params,
        )
        db.mark_done(
    ...
    # qNEHVI attempted in proposer pool and then again locally; heuristic used as recovery path.
    assert q_calls["n"] >= 2
    assert h_calls["n"] >= 1
    ...
    assert meta["method"] == "heuristic_after_pool_fail"
    assert (run_dir / "export" / "trials.csv").exists()


def test_r31dp_ray_pool_and_local_proposers_failure_fall_back_to_random(
    tmp_path: Path,
    monkeypatch,
) -> None:
    fake_ray = _make_fake_ray_module()
    ...
    db_path = tmp_path / "experiments.sqlite"
    run_dir = tmp_path / "run_ray_pool_and_local_fail_to_random"
    run_dir.mkdir(parents=True, exist_ok=True)
    ...
    core = _DummyCore(dim=2)
    with ExperimentDB(db_path, engine="sqlite") as db:
        run_id = db.create_run(problem_hash="ph_r31dp_random", spec={}, meta={"source": "test"})
        seed_x = [0.12, 0.88]
        seed_params = core.u_to_params(seed_x)
        seed_res = db.reserve_trial(
            run_id=run_id,
            problem_hash="ph_r31dp_random",
            param_hash="pool_fail_seed_done_random",
            x_u=list(seed_x),
            params=seed_params,
        )
        db.mark_done(
...
def test_r31dp_ray_pool_failure_then_local_qnehvi_success_keeps_qnehvi_mode(
    tmp_path: Path,
    monkeypatch,
) -> None:
    fake_ray = _make_fake_ray_module()
    ...
    def fake_propose_qnehvi(**kwargs):  # noqa: ARG001
        q_calls["n"] += 1
        if q_calls["n"] == 1:
            raise RuntimeError("forced pool qnehvi failure once")
        return SimpleNamespace(
            X=np.asarray([[0.21, 0.51]], dtype=float),
            meta={"method": "qnehvi_local_after_pool_fail", "source": "r31dp_test"},
        )
    ...
    db_path = tmp_path / "experiments.sqlite"
    run_dir = tmp_path / "run_ray_pool_fail_then_local_qnehvi"
    run_dir.mkdir(parents=True, exist_ok=True)
    ...
    core = _DummyCore(dim=2)
    with ExperimentDB(db_path, engine="sqlite") as db:
        run_id = db.create_run(problem_hash="ph_r31dp_local_qnehvi", spec={}, meta={"source": "test"})
        seed_x = [0.12, 0.88]
        seed_params = core.u_to_params(seed_x)
        seed_res = db.reserve_trial(
            run_id=run_id,
            problem_hash="ph_r31dp_local_qnehvi",
            param_hash="pool_fail_seed_done_local_qnehvi",
            x_u=list(seed_x),
            params=seed_params,
        )
        db.mark_done(
    ...
    assert meta["method"] == "qnehvi_local_after_pool_fail"
