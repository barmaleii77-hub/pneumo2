...
def test_r31dq_dask_portfolio_mode_uses_local_qnehvi_and_random_blend(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    def fake_propose_qnehvi(**kwargs):  # noqa: ARG001
        ...
        return SimpleNamespace(
            X=np.asarray([[0.40, 0.60]], dtype=float),
            meta={"method": "qnehvi_portfolio_local", "source": "r31dq_test"},
        )
    ...
    def fake_propose_random(*, d, q, seed=0):  # noqa: ARG001
        ...
        q_i = max(1, int(q))
        d_i = max(1, int(d))
        X = np.tile(np.asarray([[0.91, 0.92]], dtype=float), (q_i, 1))
        if d_i != 2:
    ...
    run_dir = tmp_path / "run_dask_portfolio_local"
    ...
    with ExperimentDB(db_path, engine="sqlite") as db:
        run_id = db.create_run(problem_hash="ph_r31dq", spec={}, meta={"source": "test"})
        seed_x = [0.10, 0.90]
        seed_params = core.u_to_params(seed_x)
        seed_res = db.reserve_trial(
            run_id=run_id,
            problem_hash="ph_r31dq",
            param_hash="portfolio_local_seed_done",
            x_u=list(seed_x),
            params=seed_params,
        )
        db.mark_done(
    ...
    assert q_calls["n"] >= 1
    assert rand_calls["n"] >= 1  # portfolio blend
    ...
    assert meta["method"] == "qnehvi_portfolio_local"
