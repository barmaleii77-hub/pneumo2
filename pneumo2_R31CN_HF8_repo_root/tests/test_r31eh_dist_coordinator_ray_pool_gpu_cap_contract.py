...
def test_r31eh_ray_qnehvi_pool_actor_count_is_capped_by_gpu_budget(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    def fake_propose_qnehvi(**kwargs):  # noqa: ARG001
        ...
        return SimpleNamespace(
            X=np.asarray([[0.37, 0.67]], dtype=float),
            meta={"method": "qnehvi_pool_gpu_cap", "source": "r31eh_test"},
        )
    ...
    def fake_propose_random(*, d, q, seed=0):  # noqa: ARG001
        ...
        q_i = max(1, int(q))
        d_i = max(1, int(d))
        X = np.full((q_i, d_i), 0.9, dtype=float)
    ...
    run_dir = tmp_path / "run_ray_qnehvi_pool_gpu_cap"
    ...
    args = SimpleNamespace(
        ...
        ray_num_proposers=4,  # intentionally above GPU budget
        ...
        botorch_maxiter=200,
        heuristic_pool_size=750,
        heuristic_explore=0.38,
    )

    core = _DummyCore(dim=2)
    with ExperimentDB(db_path, engine="sqlite") as db:
        run_id = db.create_run(problem_hash="ph_r31eh", spec={}, meta={"source": "test"})
        seed_x = [0.16, 0.86]
        seed_params = core.u_to_params(seed_x)
        seed_res = db.reserve_trial(
            run_id=run_id,
            problem_hash="ph_r31eh",
            param_hash="qnehvi_pool_gpu_cap_seed_done",
            x_u=list(seed_x),
            params=seed_params,
        )
        db.mark_done(
    ...
    # With 1 GPU and 1 GPU per proposer, pool actor count must cap to 1.
    assert q_calls["n"] == 1
    assert h_calls["n"] == 0
    ...
    assert meta["method"] == "qnehvi_pool_gpu_cap"
