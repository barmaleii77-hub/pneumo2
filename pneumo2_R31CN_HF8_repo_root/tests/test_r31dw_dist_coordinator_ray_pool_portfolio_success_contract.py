...
def test_r31dw_ray_portfolio_pool_success_uses_pool_qnehvi_and_random_blend(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    def fake_propose_qnehvi(**kwargs):  # noqa: ARG001
        ...
        return SimpleNamespace(
            X=np.asarray([[0.31, 0.61]], dtype=float),
            meta={"method": "portfolio_pool_qnehvi_success", "source": "r31dw_test"},
        )

    def fake_propose_heuristic(**kwargs):  # noqa: ARG001
        h_calls["n"] += 1
        raise AssertionError("heuristic must not be used on successful portfolio pool path")

    def fake_propose_random(*, d, q, seed=0):  # noqa: ARG001
        rand_calls["n"] += 1
        q_i = max(1, int(q))
        d_i = max(1, int(d))
        X = np.full((q_i, d_i), 0.98, dtype=float)
    ...
    run_dir = tmp_path / "run_ray_portfolio_pool_success"
    ...
    with ExperimentDB(db_path, engine="sqlite") as db:
        run_id = db.create_run(problem_hash="ph_r31dw", spec={}, meta={"source": "test"})
        seed_x = [0.10, 0.90]
        seed_params = core.u_to_params(seed_x)
        seed_res = db.reserve_trial(
            run_id=run_id,
            problem_hash="ph_r31dw",
            param_hash="portfolio_pool_success_seed_done",
            x_u=list(seed_x),
            params=seed_params,
        )
        db.mark_done(
    ...
    assert meta["method"] == "portfolio_pool_qnehvi_success"
