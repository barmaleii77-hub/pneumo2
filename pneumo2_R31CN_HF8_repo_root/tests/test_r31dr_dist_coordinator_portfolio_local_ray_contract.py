...
def _make_fake_ray_module() -> ModuleType:
    ...
    def cluster_resources() -> Dict[str, float]:
        # GPU=0 keeps proposer_actors empty, forcing local proposer path.
...
def test_r31dr_ray_portfolio_mode_uses_local_qnehvi_and_random_blend(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    def fake_propose_qnehvi(**kwargs):  # noqa: ARG001
        ...
        return SimpleNamespace(
            X=np.asarray([[0.35, 0.65]], dtype=float),
            meta={"method": "qnehvi_portfolio_local_ray", "source": "r31dr_test"},
        )
    ...
    def fake_propose_random(*, d, q, seed=0):  # noqa: ARG001
        ...
        q_i = max(1, int(q))
        d_i = max(1, int(d))
        X = np.tile(np.asarray([[0.93, 0.94]], dtype=float), (q_i, 1))
        if d_i != 2:
    ...
    run_dir = tmp_path / "run_ray_portfolio_local"
    ...
    args = SimpleNamespace(
        ...
        heuristic_explore=0.44,
    )

    core = _DummyCore(dim=2)
    with ExperimentDB(db_path, engine="sqlite") as db:
        run_id = db.create_run(problem_hash="ph_r31dr", spec={}, meta={"source": "test"})
        seed_x = [0.11, 0.89]
        seed_params = core.u_to_params(seed_x)
        seed_res = db.reserve_trial(
            run_id=run_id,
            problem_hash="ph_r31dr",
            param_hash="portfolio_local_seed_done_ray",
            x_u=list(seed_x),
            params=seed_params,
        )
        db.mark_done(
    ...
    assert meta["method"] == "qnehvi_portfolio_local_ray"
