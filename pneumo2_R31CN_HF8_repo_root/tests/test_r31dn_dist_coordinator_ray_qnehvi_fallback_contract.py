...
def _base_args() -> SimpleNamespace:
    return SimpleNamespace(
        ...
        botorch_maxiter=200,
        heuristic_pool_size=500,
        heuristic_explore=0.39,
    )
...
def _seed_one_done_trial(db: ExperimentDB, run_id: str, problem_hash: str, core: _DummyCore) -> None:
    ...
    res_done = db.reserve_trial(
        ...
        param_hash="ray_qnehvi_fallback_seed_done",
        x_u=list(x_done),
        params=params_done,
    )
    db.mark_done(
...
def test_r31dn_ray_qnehvi_failure_falls_back_to_heuristic(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    def fake_propose_heuristic(**kwargs):
        ...
        return SimpleNamespace(
            ...
            meta={"method": "heuristic_after_qfail_ray", "source": "r31dn_test"},
        )
    ...
    run_dir = tmp_path / "run_ray_qfail_to_heur"
    ...
    with ExperimentDB(db_path, engine="sqlite") as db:
        run_id = db.create_run(problem_hash="ph_r31dn_qh", spec={}, meta={"source": "test"})
        _seed_one_done_trial(db, run_id, "ph_r31dn_qh", core)
        coord._run_ray(
            args,
            core_local=core,
            db=db,
    ...
    assert meta["method"] == "heuristic_after_qfail_ray"
    assert (run_dir / "export" / "trials.csv").exists()


def test_r31dn_ray_qnehvi_and_heuristic_failure_fall_back_to_random(
    tmp_path: Path,
    monkeypatch,
) -> None:
    fake_ray = _make_fake_ray_module()
    ...
    db_path = tmp_path / "experiments.sqlite"
    run_dir = tmp_path / "run_ray_qh_fail_to_random"
    run_dir.mkdir(parents=True, exist_ok=True)
    ...
    with ExperimentDB(db_path, engine="sqlite") as db:
        run_id = db.create_run(problem_hash="ph_r31dn_qhr", spec={}, meta={"source": "test"})
        _seed_one_done_trial(db, run_id, "ph_r31dn_qhr", core)
        coord._run_ray(
            args,
            core_local=core,
            db=db,
