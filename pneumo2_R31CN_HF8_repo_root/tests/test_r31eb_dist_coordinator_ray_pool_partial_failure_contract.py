...
def test_r31eb_ray_pool_partial_failure_keeps_successful_pool_candidates_without_local_fallback(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    def fake_propose_qnehvi(**kwargs):  # noqa: ARG001
        q_calls["n"] += 1
        if q_calls["n"] == 1:
            raise RuntimeError("forced first pool actor failure")
        return SimpleNamespace(
            X=np.asarray([[0.24, 0.64]], dtype=float),
            meta={"method": "qnehvi_pool_partial_success", "source": "r31eb_test"},
        )
    ...
    def fake_propose_random(*, d, q, seed=0):  # noqa: ARG001
        ...
        q_i = max(1, int(q))
        d_i = max(1, int(d))
        X = np.full((q_i, d_i), 0.77, dtype=float)
    ...
    run_dir = tmp_path / "run_ray_pool_partial_failure_successful_candidate"
    ...
    # Exactly two qNEHVI calls from two pool actors; no extra local qNEHVI fallback call.
    assert q_calls["n"] == 2
    assert h_calls["n"] == 0
    ...
    assert meta["method"] == "qnehvi_pool_partial_success"
