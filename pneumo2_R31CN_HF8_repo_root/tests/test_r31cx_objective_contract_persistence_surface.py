...
def test_r31cx_coordinator_and_ui_surface_objective_contract_persistence() -> None:
    ...
    assert 'Objective contract artifact:' in page_src
    assert 'contract_diff_bits' in page_src
    assert 'resume/cache уже различает такие контракты по problem_hash' in page_src
