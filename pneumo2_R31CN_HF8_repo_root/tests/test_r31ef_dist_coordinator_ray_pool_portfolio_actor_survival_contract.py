...
def test_r31ef_ray_portfolio_pool_partial_failure_preserves_surviving_actor_for_next_cycle(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    def fake_propose_qnehvi(**kwargs):  # noqa: ARG001
        q_calls["n"] += 1
        if q_calls["n"] == 1:
            raise RuntimeError("forced first portfolio actor failure")
        return SimpleNamespace(
            X=np.asarray([[0.36, 0.66]], dtype=float),
            meta={"method": "portfolio_pool_actor_survival", "source": "r31ef_test"},
        )
    ...
    def fake_propose_random(*, d, q, seed=0):  # noqa: ARG001
        ...
        q_i = max(1, int(q))
        d_i = max(1, int(d))
        X = np.full((q_i, d_i), 0.89, dtype=float)
    ...
    run_dir = tmp_path / "run_ray_portfolio_pool_actor_survival"
    ...
    args = SimpleNamespace(
        ...
        botorch_maxiter=200,
        heuristic_pool_size=700,
        heuristic_explore=0.34,
    )

    core = _DummyCore(dim=2)
    with ExperimentDB(db_path, engine="sqlite") as db:
        run_id = db.create_run(problem_hash="ph_r31ef", spec={}, meta={"source": "test"})
        seed_x = [0.14, 0.84]
        seed_params = core.u_to_params(seed_x)
        seed_res = db.reserve_trial(
            run_id=run_id,
            problem_hash="ph_r31ef",
            param_hash="portfolio_pool_actor_survival_seed_done",
            x_u=list(seed_x),
            params=seed_params,
        )
        db.mark_done(
    ...
    # First fill: 2 calls (1 fail + 1 success). Second fill: surviving actor only.
    assert q_calls["n"] == 3
    assert h_calls["n"] == 0
    # Portfolio blend should be applied on both successful fill cycles.
    ...
    assert meta["method"] == "portfolio_pool_actor_survival"
