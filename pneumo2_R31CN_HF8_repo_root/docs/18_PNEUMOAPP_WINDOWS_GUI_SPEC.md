...
- `Desktop Animator`, `Compare Viewer` и `Desktop Mnemo` остаются специализированными окнами, но в active route они должны быть дочерними `dock/floating`-поверхностями одного `WIN-MAIN-SHELL`; запуск standalone legacy-процессов допустим только как явный сервисный fallback;
...
### Главное окно и дочерние dock-окна

- Эталон оконной структуры для active GUI path - `Desktop Animator`: один `QMainWindow`, меню `Окна`, дочерние `QDockWidget`-панели с `movable / floatable / closable` поведением и сохранением layout.
- Клик в левом дереве маршрута или выбор в command search не должен вести через промежуточную кнопку запуска. Он открывает или фокусирует соответствующую dock-панель рабочего этапа внутри главного shell.
- `Исходные данные`, `Редактор циклического сценария`, `Набор испытаний`, `Базовый прогон`, `Оптимизация`, `Анализ результатов`, `Анимация`, `Диагностика` и `Инструменты` имеют собственные дочерние dock-поверхности. Центральная область не должна становиться второй конкурирующей навигационной оболочкой.
- Меню `Окна` является меню управления dock-панелями: один toggle/action на одну дочернюю поверхность, без дублирующей навигации и без второго списка тех же этапов.
- Подробные проверки вроде сравнения результатов, проверки движения и мнемосхемы открываются как дочерние dock/floating-панели active shell. Старые standalone windows сохраняются только как `support_fallback` для восстановления и сравнения поведения.
