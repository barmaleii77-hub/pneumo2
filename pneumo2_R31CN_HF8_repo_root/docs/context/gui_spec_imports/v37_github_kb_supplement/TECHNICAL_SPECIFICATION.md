...
## Package status V37
Этот документ входит в successor repo-adoption package V37. Исторические упоминания `v30`, `V32`, `V33` ниже относятся к lineage и provenance. Пакет не заявляет runtime-closure proof без отдельного runtime evidence слоя.


## 0. Repo adoption layer
V37 не меняет продуктовый канон `17/18`, а переводит consolidated spec в import-ready successor layer для репозитория.
