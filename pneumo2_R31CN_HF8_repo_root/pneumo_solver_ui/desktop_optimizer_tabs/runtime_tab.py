...
class DesktopOptimizerRuntimeTab(ttk.Frame):
    def __init__(self, master: tk.Misc, controller: object) -> None:
        ...
        ttk.Label(
            body,
            text=(
                "Здесь собраны stage policy, distributed knobs, live log и запуск StageRunner / coordinator без WEB."
            ),
            wraplength=980,
            justify="left",
        ...
        ttk.Combobox(
            ...
            values=list(launch_profile_labels()),
            state="readonly",
            width=34,
        ...
        ttk.Label(
            profile_frame,
            text=(
                "Profiles быстро перенастраивают runtime knobs под triage, validation, Dask explore или Ray handoff."
            ),
            foreground="#555555",
            wraplength=920,
            justify="left",
        ...
        self._add_entry(staged_frame, row=0, col=0, label="Minutes", var_name="ui_opt_minutes")
        self._add_entry(staged_frame, row=0, col=2, label="Jobs", var_name="ui_jobs")
        ...
        self._add_entry(staged_frame, row=3, col=0, label="Stop pen s1", var_name="stop_pen_stage1")
        self._add_entry(staged_frame, row=3, col=2, label="Stop pen s2", var_name="stop_pen_stage2")
        ttk.Checkbutton(
            staged_frame,
