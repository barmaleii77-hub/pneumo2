...
class DesktopOptimizerPackagingTab(ttk.Frame):
    def __init__(self, master: tk.Misc, controller: object) -> None:
        ...
        ttk.Button(actions, text="Обновить packaging", command=controller.refresh_packaging).pack(side="left")
        ...
        filters = ttk.LabelFrame(right, text="Packaging filters", padding=8)
        ...
        ttk.Checkbutton(
            filters,
            text="Zero interference only",
            variable=controller.var("opt_packaging_zero_interference_only"),
            command=controller.refresh_packaging,
        ).grid(row=1, column=1, columnspan=2, sticky="w", pady=(8, 0), padx=(8, 0))
        ttk.Button(
            filters,
            text="Apply packaging filters",
            command=controller.refresh_packaging,
        ).grid(row=0, column=4, rowspan=2, sticky="e", padx=(16, 0))

        self.overview_panel = TextReportPanel(right, text="Packaging overview", height=8)
        self.overview_panel.grid(row=2, column=0, sticky="ew", pady=(10, 0))
        self.ranking_panel = TextReportPanel(right, text="Readiness ranking", height=10)
        self.ranking_panel.grid(row=3, column=0, sticky="ew", pady=(10, 0))
        self.snapshot_panel = TextReportPanel(right, text="Selected packaging snapshot", height=10)
        self.snapshot_panel.grid(row=4, column=0, sticky="ew", pady=(10, 0))
        self.contract_panel = TextReportPanel(right, text="Selected packaging contract", height=9)
