# -*- coding: utf-8 -*-
"""pipeline_npz_autopilot_v8.py

Autopilot "всё автоматически" для калибровки по NPZ + multiobjective.

v8 — инженерное усиление для многопараметрических задач:
...
python calibration/pipeline_npz_autopilot_v8.py --osc_dir <OSC_DIR> --run_oed --run_profile_auto --run_plots --run_pareto --run_epsilon

Выход: calibration_runs/RUN_..._autopilot_v8/
...
def main():
    ...
    # staging parameters
    ap.add_argument("--param_staging", default="auto", choices=["off", "auto", "heuristic", "sensitivity"],
    ...
    if args.out_dir:
        ...
        out_dir = project_root / "calibration_runs" / f"RUN_{ts}_autopilot_v8"
    ...
    cmd_iter = [
        sys.executable, str(project_root / "calibration" / "pipeline_npz_iterative_signals_v2.py"),
        "--osc_dir", str(osc_dir),
    ...
    print("\nDONE. Autopilot v8 outputs in:", out_dir)
