...
Запуск (из корня pneumo_v7):
  python calibration/pipeline_npz_group_balance_v2.py \
...
def update_gains(
    ...
    if strategy == "equalize_contrib":
        ...
        for g in groups:
            ...
            # average of (p*m) is 1
            mul = float(max(1e-9, p * m) ** float(alpha))
            out[g] = float(out.get(g, 1.0) * mul)
...
def main():
    ...
    # report
    md: List[str] = []
    md.append("# Group balance report (v2)")
    md.append("")
    md.append("## Settings")
    ...
    if hist_rows:
        ...
        md.append("|---|---:|---:|")
        for g in groups:
            md.append(f"| {g} | {last.get('rmse_unb_'+g)} | {last.get('gain_'+g)} -> {last.get('gain_new_'+g)} |")
