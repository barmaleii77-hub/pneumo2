...
python calibration/pipeline_npz_autopilot_v6.py --osc_dir <OSC_DIR> --run_oed --run_profile_auto --run_plots

Выход: calibration_runs/RUN_..._autopilot_v5/
...
def main():
    ...
    if args.out_dir:
        ...
        out_dir = project_root / "calibration_runs" / f"RUN_{ts}_autopilot_v5"
    ...
    if getattr(args, 'run_epsilon', False):
        ...
        cmd_eps = [
            sys.executable, str(project_root / 'calibration' / 'pipeline_npz_epsilon_constraint_v1.py'),
            '--osc_dir', str(osc_dir),
            '--signals_csv', str(out_dir / 'FINAL_SIGNALS.csv'),
    ...
    if (args.run_pareto or getattr(args, 'run_epsilon', False)) and (not bool(getattr(args, 'skip_tradeoff_decision', False))):
        cmd_dec = [
            sys.executable, str(project_root / "calibration" / "tradeoff_decision_support_v1.py"),
            "--run_dir", str(out_dir),
    ...
    # summary
    summary = []
    summary.append("# Autopilot v6 summary\n")
