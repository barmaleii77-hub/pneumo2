...
python calibration/pipeline_npz_autopilot_v14.py --osc_dir <OSC_DIR> --run_oed --run_profile_auto --run_plots --run_pareto --run_epsilon

Выход: calibration_runs/RUN_..._autopilot_v14/
...
def main():
    ...
    if args.out_dir:
        ...
        out_dir = project_root / "calibration_runs" / f"RUN_{ts}_autopilot_v14"
    ...
    # 1c) adaptive group balance: tune group_weights to equalize group errors (optional)
    group_balance_dir = out_dir / "group_balance"
    if args.run_group_balance and _budget_ok():
        cmd_gb = [
            sys.executable, str(project_root / "calibration" / "pipeline_npz_group_balance_v1.py"),
            "--osc_dir", str(osc_dir),
            "--signals_csv", str(final_signals),
