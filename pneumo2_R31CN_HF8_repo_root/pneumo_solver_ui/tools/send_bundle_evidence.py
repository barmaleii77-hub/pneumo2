...
try:
    from pneumo_solver_ui.optimization_baseline_source import describe_active_baseline_state
except ImportError:
        ...
        *,
        ...
        if not payload:
            return {
                ...
                "banner": "Active baseline contract is missing.",
            }
        stale_reasons: list[str] = []
        if current_suite_snapshot_hash and str(payload.get("suite_snapshot_hash") or "") != current_suite_snapshot_hash:
            stale_reasons.append("suite_snapshot_hash_mismatch")
        if current_inputs_snapshot_hash and str(payload.get("inputs_snapshot_hash") or "") != current_inputs_snapshot_hash:
            stale_reasons.append("inputs_snapshot_hash_mismatch")
        if current_ring_source_hash and str(payload.get("ring_source_hash") or "") != current_ring_source_hash:
            stale_reasons.append("ring_source_hash_mismatch")
        if stale_reasons:
            return {
                "state": "stale",
                "is_stale": True,
                "optimizer_can_consume": False,
                "banner_id": "BANNER-BASELINE-STALE-001",
                "stale_reasons": stale_reasons,
                "banner": "Active baseline contract does not match the current context.",
            }
        return {
            "state": "current",
            "is_stale": False,
            ...
            "banner_id": "",
            "stale_reasons": [],
            "banner": "Active baseline contract matches the current context.",
        }

try:
    from pneumo_solver_ui.optimization_baseline_source import compare_active_and_historical_baseline
except ImportError:

    def _baseline_contract_view(value: Mapping[str, Any] | None) -> dict[str, Any]:
        src = dict(value or {})
        for key in ("contract", "baseline_contract", "active_baseline_contract", "payload"):
            nested = src.get(key)
            if isinstance(nested, Mapping):
                return dict(nested)
        return src

    def _baseline_compare_value(value: Mapping[str, Any], key: str) -> str:
        if key == "policy_mode":
    ...
    def compare_active_and_historical_baseline(
        ...
        active = _baseline_contract_view(active_contract)
        historical = _baseline_contract_view(historical_item)
        if not active or not historical:
            return {
                "state": "missing",
                "banner_id": "BANNER-HIST-003",
                "mismatch_fields": tuple(),
                "banner": "Active or historical baseline contract payload is missing.",
            }
        ...
        if mismatch_fields:
            return {
                ...
                "banner": "Historical baseline was produced from a different context.",
            }
        ...
        if same_hash:
            return {
                ...
                "banner_id": "",
                "mismatch_fields": tuple(),
                "banner": "Historical baseline matches active baseline contract.",
            }
        return {
            "state": "historical_same_context",
            "banner_id": "BANNER-HIST-001",
            "mismatch_fields": tuple(),
            "banner": "Historical baseline is from the same context and requires explicit action.",
        }
