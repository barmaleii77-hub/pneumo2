...
def _launch_animator(*, theme: str, follow: bool = False, no_gl: bool = False) -> tuple[bool, str]:
    py = _venv_python(prefer_gui=True)
    cmd = [py, "-m", "pneumo_solver_ui.desktop_animator.main"]
    if follow:
    ...
    if follow and no_gl:
        return True, "Desktop Animator запущен в follow-режиме с low-load preset (--no-gl)."
    if follow:
        return True, "Запущено (если система позволяет GUI)."
    if no_gl:
        return True, "Desktop Animator запущен в low-load режиме (--no-gl)."
...
if POINTER_PATH.exists():
    try:
        ...
        if pointer_npz_path and Path(pointer_npz_path).exists():
            st.caption(f"NPZ размер: {Path(pointer_npz_path).stat().st_size / 1024.0 / 1024.0:.2f} MB")
        else:
...
perf_related_recommendations = [
    str(x)
    for x in operator_recommendations
    if "browser perf" in str(x).lower()
]
animator_follow_ready = bool(POINTER_PATH.exists() and pointer_npz_path and pointer_npz_path.exists())
animator_safe_no_gl = bool(
    no_gl
    or (platform.system() == "Windows" and not has_gl_accel)
    or perf_related_recommendations
)
animator_preset_label = "Запустить preset: пустой Animator"
animator_preset_reason = (
    "Свежий anim_latest пока не готов, поэтому safest preset — пустой старт без follow."
)
animator_preset_follow = False
animator_preset_no_gl = animator_safe_no_gl

if animator_follow_ready and animator_safe_no_gl:
    animator_preset_label = "Запустить preset: follow с low-load (--no-gl)"
    animator_preset_reason = (
        "Launcher видит perf/OpenGL-риск: для первого запуска лучше follow с --no-gl, чтобы снизить нагрузку и ускорить triage."
    )
    animator_preset_follow = True
elif animator_follow_ready:
    animator_preset_label = "Запустить preset: multi-view follow"
    animator_preset_reason = (
        "Свежий anim_latest готов, а perf/OpenGL-ограничений не видно: можно запускать полноценный follow с multi-view."
    )
    animator_preset_follow = True
elif animator_safe_no_gl:
    animator_preset_label = "Запустить preset: пустой low-load Animator"
    animator_preset_reason = (
        "Follow пока не готов, но safe-mode всё равно полезен: откройте окно в low-load режиме и затем загрузите нужный сценарий вручную."
    )

st.info(animator_preset_reason)
if st.button(animator_preset_label, width="stretch"):
    ok, msg = _launch_animator(
        theme=str(theme),
        follow=animator_preset_follow,
        no_gl=animator_preset_no_gl,
    )
    if ok:
        st.success(msg)
    else:
...
with cA:
    if st.button("Запустить Desktop Animator (follow)"):
        ok, msg = _launch_animator(theme=str(theme), follow=True, no_gl=bool(no_gl))
        if ok:
            st.success(msg)
        else:
            st.error(msg)
...
if st.button("Запустить Desktop Animator (пустой)"):
    ok, msg = _launch_animator(theme=str(theme), no_gl=bool(no_gl))
    if ok:
        st.success(msg)
    else:
        st.error(msg)
