...
st.caption(
    "Здесь собраны общие настройки приложения: диагностика, производительность интерфейса, пути хранения данных. "
    "Часто используемые настройки также должны быть продублированы в тематических разделах."
)

tab_diag, tab_perf, tab_paths = st.tabs(["Диагностика", "Производительность UI", "Пути и хранение"])


with tab_diag:
    st.subheader("Диагностика")
    st.write(
        "Кнопка «Диагностика» в сайдбаре сохраняет ZIP‑архив со всем нужным для анализа: "
    ...
    with c1:
        ...
        st.number_input(
            ...
            help="Крупные бинарные файлы могут раздувать архив. Ограничение помогает сделать ZIP отправляемым.",
        )
    ...
    with c2:
        ...
        st.checkbox(
            "Автосохранение диагностики при выходе",
            key="settings_diag_autosave_on_exit",
            help="Сохранять диагностику автоматически при закрытии приложения.",
        )
        st.checkbox(
            "Автосохранение диагностики при краше",
    ...
    st.info(
        "Папка и настройки диагностики применяются сразу. Если автосохранение включено — оно будет работать "
        "и при выходе, и при краше (насколько это возможно в среде запуска)."
    )
...
with tab_paths:
    ...
    st.write(
        "Пути используются для сохранения профилей, кэша, промежуточных результатов и диагностики. "
    ...
    st.markdown("**Диагностика (папка для ZIP):**")
    st.code(str(out_dir))
