...
# -------------------------------
# Streamlit иногда падает на declare_component() с:
#   RuntimeError: module is None. This should never happen.
# Это возникает, когда declare_component вызван из "страницы", которую Streamlit исполняет через exec().
# Фикс: объявлять компоненты из обычного импортируемого модуля (pneumo_solver_ui.ui_components).
try:
    from pneumo_solver_ui import ui_components as _ui_comp  # noqa: E402
    get_pneumo_svg_flow_component = _ui_comp.get_pneumo_svg_flow_component  # type: ignore
    get_mech_anim_component = _ui_comp.get_mech_anim_component  # type: ignore
    get_mech_car3d_component = _ui_comp.get_mech_car3d_component  # type: ignore
    get_playhead_ctrl_component = _ui_comp.get_playhead_ctrl_component  # type: ignore
    _emit("ComponentFactoryRebind", "Using pneumo_solver_ui.ui_components for declare_component()", module="pneumo_solver_ui.ui_components")
except Exception as _e_compbind:
    try:
        _emit("ComponentFactoryRebindFailed", repr(_e_compbind))
    except Exception:
        pass
...
def get_pneumo_svg_flow_component():
    ...
    if not comp_dir.exists():
        _emit("ComponentMissing", "pneumo_svg_flow assets missing", component="pneumo_svg_flow", path=str(comp_dir))
        _PNEUMO_SVG_FLOW_COMPONENT = None
        return _PNEUMO_SVG_FLOW_COMPONENT
    ...
    try:
        ...
        _emit("ComponentDeclared", "pneumo_svg_flow", component="pneumo_svg_flow", path=str(comp_dir.resolve()))
    except Exception as e:
        _emit("ComponentDeclareFailed", repr(e), component="pneumo_svg_flow", path=str(comp_dir.resolve()))
        _PNEUMO_SVG_FLOW_COMPONENT = None
...
_MECH_ANIM_COMPONENT_ERR = None  # repr(exception) if declare_component fails
...
def get_mech_anim_component():
    ...
    to a static image.
    """
    global _MECH_ANIM_COMPONENT, _MECH_ANIM_COMPONENT_ERR
    if _MECH_ANIM_COMPONENT is not None:
        return _MECH_ANIM_COMPONENT

    comp_dir = HERE / "components" / "mech_anim"
    if not comp_dir.exists():
        _emit("ComponentMissing", "mech_anim assets missing", component="mech_anim", path=str(comp_dir))
        _MECH_ANIM_COMPONENT_ERR = f"assets missing: {comp_dir}"
        _MECH_ANIM_COMPONENT = None
        return _MECH_ANIM_COMPONENT
    ...
    try:
        ...
        _emit("ComponentDeclared", "mech_anim", component="mech_anim", path=str(comp_dir.resolve()))
        _MECH_ANIM_COMPONENT_ERR = None
    except Exception as e:
        _emit("ComponentDeclareFailed", repr(e), component="mech_anim", path=str(comp_dir.resolve()))
        _MECH_ANIM_COMPONENT_ERR = repr(e)
        _MECH_ANIM_COMPONENT = None
...
def get_mech_car3d_component():
    """Return (and cache) the mechanical car 3D animation component callable."""
    ...
    if not comp_dir.exists():
        _emit("ComponentMissing", "mech_car3d assets missing", component="mech_car3d", path=str(comp_dir))
        _MECH_CAR3D_COMPONENT = None
        return _MECH_CAR3D_COMPONENT
    ...
    try:
        ...
        _emit("ComponentDeclared", "mech_car3d", component="mech_car3d", path=str(comp_dir.resolve()))
    except Exception as e:
        _emit("ComponentDeclareFailed", repr(e), component="mech_car3d", path=str(comp_dir.resolve()))
        _MECH_CAR3D_COMPONENT = None
...
def get_playhead_ctrl_component():
    ...
    if not comp_dir.exists():
        _emit("ComponentMissing", "playhead_ctrl assets missing", component="playhead_ctrl", path=str(comp_dir))
        _PLAYHEAD_CTRL_COMPONENT = None
        return _PLAYHEAD_CTRL_COMPONENT
    ...
    try:
        ...
        _emit("ComponentDeclared", "playhead_ctrl", component="playhead_ctrl", path=str(comp_dir.resolve()))
    except Exception as e:
        _emit("ComponentDeclareFailed", repr(e), component="playhead_ctrl", path=str(comp_dir.resolve()))
        _PLAYHEAD_CTRL_COMPONENT = None

    return _PLAYHEAD_CTRL_COMPONENT


# --- IMPORTANT: use safe Streamlit component factories ---
# Some Streamlit/Python combinations still throw
#   RuntimeError: module is None. This should never happen
# during declare_component(). We always fall back gracefully,
# but we also rebind factories here to ensure we use
# pneumo_solver_ui.ui_components (imported via normal importlib).
try:
    from pneumo_solver_ui import ui_components as _ui_comp
    get_pneumo_svg_flow_component = _ui_comp.get_pneumo_svg_flow_component  # type: ignore
    get_mech_anim_component = _ui_comp.get_mech_anim_component  # type: ignore
    get_mech_car3d_component = _ui_comp.get_mech_car3d_component  # type: ignore
    get_playhead_ctrl_component = _ui_comp.get_playhead_ctrl_component  # type: ignore
...
def make_ui_diagnostics_zip(
    ...
    *,
    ...
    with zipfile.ZipFile(out_zip_path, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        zf.writestr("meta.json", _json.dumps(_json_safe(meta), ensure_ascii=False, indent=2))
...
def _infer_unit_and_transform(col: str):
    ...
    """
    c = str(col)
    # pressures in Pa -> bar gauge (изб.)
    if c.endswith("_Па") and ("давление" in c or "p_" in c.lower()):
        return ("бар (изб.)", lambda a: (a - P_ATM) / BAR_PA, "бар (изб.)")
    # angles rad -> deg
    ...
    if c.endswith("_м") or c.endswith("_m"):
        return ("мм", lambda a: a * 1000.0, "мм")
    # forces
...
def pa_abs_to_bar_g(p_abs_pa: float) -> float:
    """Абсолютное давление (Па) -> бар (изб.) относительно P_ATM."""
    return (float(p_abs_pa) - P_ATM) / BAR_PA


def bar_g_to_pa_abs(p_g_bar: float) -> float:
    """бар (изб.) -> абсолютное давление (Па)."""
    return P_ATM + float(p_g_bar) * BAR_PA
...
def pa_abs_to_atm_g(p_abs_pa: float) -> float:
    """Абсолютное давление (Па) -> атм (изб.) относительно P_ATM (legacy)."""
    return (float(p_abs_pa) - P_ATM) / ATM_PA


def atm_g_to_pa_abs(p_g_atm: float) -> float:
    """атм (изб.) -> абсолютное давление (Па) (legacy)."""
...
def is_length_param(name: str) -> bool:
    ...
    if name in {"колея", "база", "ширина_рамы", "ход_штока", "статический_ход_колеса"}:
        return True
    return name.endswith("_м") or name.endswith("_m")
...
def _si_to_ui(key: str, x_si: Any, kind: str) -> Any:
    ...
    if kind == 'pressure_bar_g':
        # x_si: Pa absolute -> bar gauge in UI
        return (float(x_si) - P_ATM) / BAR_PA
    if kind == 'pressure_atm_g':
        # legacy: Pa absolute -> atm gauge (старый формат)
...
def _ui_to_si(key: str, x_ui: float, kind: str) -> float:
    if kind == "pressure_bar_g":
        # bar gauge -> Pa absolute
        return P_ATM + BAR_PA * float(x_ui)
    if kind == "pressure_atm_g":
        # legacy: atm gauge -> Pa absolute
        ...
        with st.spinner("Считаю..."):
            for name, test, dt_i, t_end_i, targets in tests:
                ...
                try:
                    ...
                    res_rows.append({"тест": name, "ошибка": str(e), "штраф": 1e9, "штраф_цели": 1e9, "штраф_верификация": 0.0, "pass": 0, "pass_верификация": 0, "pass_цели": 0})
