...
COMMAND_ELEMENT_BY_ID: dict[str, str] = {
    ...
    "results.compare.open": "RS-COMPARE-PICKER",
...
ROUTE_QUICK_ACTIONS_BY_WORKSPACE: dict[str, tuple[str, ...]] = {
    ...
    "results_analysis": ("results.center.open", "results.compare.prepare", "results.evidence.prepare", "workspace.animation.open", "workspace.diagnostics.open"),
    "animation": ("animation.animator.open", "animation.mnemo.open", "workspace.results_analysis.open", "workspace.diagnostics.open"),
...
def build_shell_commands() -> tuple[DesktopShellCommandSpec, ...]:
    commands = (
        ...
        DesktopShellCommandSpec("baseline.run_setup.verify", "Проверить готовность базового прогона", "Проверить, что снимок набора испытаний готов для базового прогона.", "baseline_run", "hosted_action", "Окна -> Базовый прогон -> Проверка готовности", capability_ids=("calculation.run_setup", "calculation.validation_and_prechecks"), launch_surface="workspace", status_label="Проверка готовности", help_topic_id="baseline_run", search_aliases=("проверить базовый прогон", "готовность набора", "проверка перед запуском")),
        DesktopShellCommandSpec("baseline.run_setup.prepare_checked", "Проверить и подготовить запуск", "Проверить готовность и подготовить запуск базового прогона без ухода из основного маршрута.", "baseline_run", "hosted_action", "Окна -> Базовый прогон -> Проверить и подготовить запуск", capability_ids=("calculation.run_setup", "calculation.baseline_run", "calculation.launch"), launch_surface="workspace", status_label="Проверка перед запуском", help_topic_id="baseline_run", search_aliases=("проверить и запустить", "подготовить запуск", "запуск базового прогона")),
        DesktopShellCommandSpec("baseline.run_setup.prepare", "Подготовить запуск", "Подготовить запуск базового прогона по актуальному снимку набора испытаний.", "baseline_run", "hosted_action", "Окна -> Базовый прогон -> Подготовить запуск", capability_ids=("calculation.run_setup", "calculation.baseline_run", "calculation.launch"), launch_surface="workspace", status_label="Подготовка запуска", help_topic_id="baseline_run", search_aliases=("запустить базовый прогон", "подготовить базовый прогон", "обычный запуск")),
