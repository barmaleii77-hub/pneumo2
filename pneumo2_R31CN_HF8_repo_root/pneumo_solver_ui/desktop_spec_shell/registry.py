...
WORKSPACE_CATALOG_ALIAS_CODES_BY_ID: dict[str, tuple[str, ...]] = {
    ...
    "diagnostics": ("рабочее_пространство.Проверка_и_отправка",),
...
def build_shell_workspaces() -> tuple[DesktopWorkspaceSpec, ...]:
    workspaces = (
        DesktopWorkspaceSpec(
            ...
            route_order=10,
            kind="main",
            summary="Главная инженерная сводка проекта, опорный прогон, цели оптимизации, результаты, проверка и отправка.",
            ...
            details="Здесь находятся быстрые действия, сводка проверки и видимое состояние опорного прогона, оптимизации, проверки и отправки.",
        ...
        DesktopWorkspaceSpec(
            ...
            details="Окно показывает текущий снимок набора, связи со сценариями и причины блокировок без служебных меток.",
        ...
        DesktopWorkspaceSpec(
            ...
            details="Рабочий шаг показывает условия запуска и открывает существующие окна без служебных меток.",
        ...
        DesktopWorkspaceSpec(
            ...
            details="Главное окно открывает анализ результатов и окно сравнения как согласованный рабочий шаг анализа.",
        ...
        DesktopWorkspaceSpec(
            ...
            source_of_truth="Аниматор и мнемосхема остаются отдельными специализированными окнами, запускаемыми из контекста главного окна.",
        ...
        DesktopWorkspaceSpec(
            ...
            route_order=90,
            kind="main",
            summary="Проверка проекта, архив для отправки и передача результатов.",
            source_of_truth="Проверка и отправка являются основным рабочим шагом и доступны из любого места приложения.",
            launch_surface="workspace",
            next_step="Собирайте архив для отправки главной кнопкой или открывайте проверку проекта перед передачей результатов.",
            hard_gate="Проверку и отправку нельзя прятать за второстепенными меню или неочевидными обходными путями.",
            details="Главная кнопка, быстрый поиск и карточка панели проекта обязаны вести в проверку и отправку без обходных путей; отдельное окно остаётся инструментом восстановления.",
            units_policy="Поля проверки показывают источник, время и тип материала, а не инженерные единицы.",
            graphics_policy="Сводка архива для отправки и состояние проекта обязаны честно показывать готовность и недостающие файлы.",
            capability_ids=("tools.diagnostics_and_bundle",),
            search_aliases=_aliases_for_capabilities(("tools.diagnostics_and_bundle",), "проверка", "архив для отправки", "отправка результатов"),
        ...
        DesktopWorkspaceSpec(
            ...
            next_step="Используйте справочник геометрии, проверки и связанные инструменты только в понятном контексте.",
...
def build_shell_commands() -> tuple[DesktopShellCommandSpec, ...]:
    commands = (
        DesktopShellCommandSpec("workspace.overview.open", "Открыть панель проекта", "Открыть инженерную сводку, опорный прогон, проверку, отправку и быстрые действия.", "overview", "open_workspace", "Окна -> Панель проекта", target_workspace_id="overview", search_aliases=("обзор", "главная"), help_topic_id="overview"),
        ...
        DesktopShellCommandSpec("input.editor.open", "Открыть исходные данные отдельным окном", "Запустить инженерный редактор исходных данных отдельным окном.", "input_data", "launch_module", "Окна -> Исходные данные -> Редактор исходных данных", module="pneumo_solver_ui.tools.desktop_input_editor", capability_ids=("input.project_entry_and_setup",), launch_surface="legacy_bridge", status_label="Отдельное окно", help_topic_id="input_data"),
        ...
        DesktopShellCommandSpec("ring.editor.open", "Открыть редактор циклического сценария", "Запустить редактор циклического сценария отдельным окном.", "ring_editor", "launch_module", "Окна -> Редактор циклического сценария -> Редактор", module="pneumo_solver_ui.tools.desktop_ring_scenario_editor", launch_surface="legacy_bridge", status_label="Отдельное окно", help_topic_id="ring_editor", search_aliases=("редактор циклического сценария", "циклический сценарий", "редактор кольца", "сценарии", "генерация дороги")),
        ...
        DesktopShellCommandSpec("baseline.run_setup.open", "Настроить и запустить базовый прогон", "Открыть настройку расчёта, предпросмотр дороги и запуск базового прогона после проверки набора испытаний.", "baseline_run", "launch_module", "Окна -> Базовый прогон -> Настройка и запуск", module="pneumo_solver_ui.tools.desktop_run_setup_center", capability_ids=("calculation.run_setup", "calculation.baseline_run", "calculation.launch"), launch_surface="legacy_bridge", status_label="Отдельное окно", help_topic_id="baseline_run", search_aliases=("базовый прогон", "опорный прогон", "настройка расчёта", "запуск расчёта", "предпросмотр дороги")),
        DesktopShellCommandSpec("baseline.center.open", "Открыть базовый прогон", "Перейти к базовому прогону: активный результат, история, просмотр, принятие и восстановление.", "baseline_run", "open_workspace", "Окна -> Базовый прогон -> Просмотр и управление", target_workspace_id="baseline_run", launch_surface="workspace", status_label="Встроенное окно", help_topic_id="baseline_run", search_aliases=("базовый прогон", "активный опорный прогон", "история прогона", "запустить опорный прогон", "базовый запуск")),
        DesktopShellCommandSpec("baseline.review", "Просмотреть выбранный опорный прогон", "Открыть выбранный активный или исторический прогон, матрицу расхождений и правила применения без изменения активного результата.", "baseline_run", "hosted_action", "Окна -> Базовый прогон -> Просмотр опорного прогона", status_label="Только просмотр", help_topic_id="baseline_run", automation_id="BL-BTN-REVIEW", search_aliases=("просмотр опорного прогона", "проверить опорный прогон", "матрица расхождений", "история опорного прогона")),
        ...
        DesktopShellCommandSpec("optimization.center.open", "Открыть оптимизацию отдельным окном", "Запустить окно оптимизации: основной поэтапный режим и расширенная распределённая координация.", "optimization", "launch_module", "Окна -> Оптимизация -> Настройка и запуск", module="pneumo_solver_ui.tools.desktop_optimizer_center", capability_ids=("optimization.orchestration_and_databases",), launch_surface="legacy_bridge", status_label="Отдельное окно", help_topic_id="optimization", search_aliases=("поэтапный запуск", "распределённая координация", "решатель")),
        ...
        DesktopShellCommandSpec("results.center.open", "Открыть анализ результатов", "Запустить окно анализа результатов.", "results_analysis", "launch_module", "Окна -> Анализ результатов -> Графики и проверка", module="pneumo_solver_ui.tools.desktop_results_center", capability_ids=("results.compare_and_review", "analysis.influence_and_exploration"), launch_surface="legacy_bridge", status_label="Отдельное окно", help_topic_id="results_analysis", search_aliases=("результаты", "анализ", "проверка")),
        DesktopShellCommandSpec("results.compare.open", "Сравнить прогоны", "Запустить окно подробного сравнения из анализа результатов.", "results_analysis", "launch_module", "Окна -> Анализ результатов -> Окно сравнения", module="pneumo_solver_ui.qt_compare_viewer", capability_ids=("results.compare_and_review",), launch_surface="external_window", help_topic_id="results_analysis", search_aliases=("сравнение", "сравнение прогонов", "окно сравнения")),
        ...
        DesktopShellCommandSpec("animation.animator.open", "Запустить аниматор", "Запустить отдельное окно анимации с приоритетом визуальной проверки.", "animation", "launch_module", "Окна -> Анимация -> Аниматор", module="pneumo_solver_ui.desktop_animator.app", capability_ids=("visualization.animator_and_mnemo",), launch_surface="external_window", help_topic_id="animation", search_aliases=("аниматор", "анимация", "трёхмерный вид")),
        DesktopShellCommandSpec("animation.mnemo.open", "Открыть мнемосхему", "Запустить отдельное окно мнемосхемы из контекста результата.", "animation", "launch_module", "Окна -> Анимация -> Мнемосхема", module="pneumo_solver_ui.desktop_mnemo.main", capability_ids=("visualization.animator_and_mnemo",), launch_surface="external_window", help_topic_id="animation", search_aliases=("мнемосхема", "пневмосхема")),
        DesktopShellCommandSpec("workspace.diagnostics.open", "Перейти к проверке и отправке", "Перейти к проверке проекта, архиву для отправки и передаче результатов.", "diagnostics", "open_workspace", "Окна -> Проверка и отправка", target_workspace_id="diagnostics", capability_ids=("tools.diagnostics_and_bundle",), help_topic_id="diagnostics"),
        DesktopShellCommandSpec("diagnostics.collect_bundle", "Собрать архив для отправки", "Открыть проверку проекта и запустить сборку архива через общее действие приложения.", "diagnostics", "hosted_action", "Окна -> Проверка и отправка -> Собрать архив для отправки", capability_ids=("tools.diagnostics_and_bundle",), help_topic_id="diagnostics", search_aliases=("собрать архив", "архив для отправки", "проверка"), status_label="Всегда доступно"),
        DesktopShellCommandSpec("diagnostics.verify_bundle", "Проверить архив для отправки", "Обновить проверку состава и состояния архива для отправки.", "diagnostics", "hosted_action", "Окна -> Проверка и отправка -> Проверить архив", capability_ids=("tools.diagnostics_and_bundle",), help_topic_id="diagnostics", search_aliases=("проверить архив", "архив для отправки", "проверка архива"), status_label="Проверка и отправка"),
        DesktopShellCommandSpec("diagnostics.send_results", "Отправить результаты", "Открыть отправку результатов после проверки актуального архива.", "diagnostics", "hosted_action", "Окна -> Проверка и отправка -> Отправить результаты", capability_ids=("tools.diagnostics_and_bundle",), help_topic_id="diagnostics", search_aliases=("отправить архив", "отправить результаты", "передать результаты"), status_label="Проверка и отправка"),
        DesktopShellCommandSpec("diagnostics.legacy_center.open", "Открыть проверку и отправку отдельным окном", "Запустить проверку и отправку отдельным окном.", "diagnostics", "launch_module", "Окна -> Проверка и отправка -> Отдельное окно", module="pneumo_solver_ui.tools.desktop_diagnostics_center", capability_ids=("tools.diagnostics_and_bundle",), launch_surface="legacy_bridge", help_topic_id="diagnostics", search_aliases=("проверка отдельным окном", "отправка отдельным окном"), status_label="Отдельное окно"),
        DesktopShellCommandSpec("workspace.app_settings.open", "Перейти к параметрам приложения", "Перейти к редким настройкам приложения и главного окна.", "app_settings", "open_workspace", "Окна -> Параметры приложения", target_workspace_id="app_settings", help_topic_id="app_settings"),
        ...
        DesktopShellCommandSpec("tools.geometry_reference.open", "Открыть справочник геометрии", "Запустить справочник геометрии как специализированное окно.", "tools", "launch_module", "Окна -> Инструменты -> Справочник геометрии", module="pneumo_solver_ui.tools.desktop_geometry_reference_center", capability_ids=("reference.geometry_and_guides", "analysis.influence_and_exploration"), launch_surface="tooling", help_topic_id="tools", search_aliases=("геометрия", "справочник", "reference center")),
        DesktopShellCommandSpec("tools.autotest.open", "Открыть проверки", "Запустить вспомогательные автономные проверки.", "tools", "launch_module", "Окна -> Инструменты -> Проверки", module="pneumo_solver_ui.tools.run_autotest_gui", launch_surface="tooling", help_topic_id="tools", search_aliases=("autotest", "автотест", "проверки")),
        ...
        DesktopShellCommandSpec("tools.spec_shell.open", "Открыть проверочное рабочее место", "Запустить проверочное рабочее место для сверки порядка работы и доступности рабочих окон.", "tools", "launch_module", "Окна -> Инструменты -> Проверочное рабочее место", module="pneumo_solver_ui.tools.desktop_gui_spec_shell", launch_surface="tooling", help_topic_id="tools", search_aliases=("проверочное рабочее место", "сверка рабочих окон")),
        DesktopShellCommandSpec("tools.legacy_shell.open", "Открыть рабочие окна во вкладках", "Открыть отдельное окно с вкладками для доступа к рабочим окнам проекта.", "tools", "launch_module", "Окна -> Инструменты -> Рабочие окна во вкладках", module="pneumo_solver_ui.tools.desktop_main_shell", launch_surface="tooling", help_topic_id="tools", search_aliases=("рабочие окна во вкладках", "вкладки рабочих окон"), status_label="Отдельное окно"),
    )
