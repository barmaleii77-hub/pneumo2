...
def build_shell_workspaces() -> tuple[DesktopWorkspaceSpec, ...]:
    workspaces = (
        DesktopWorkspaceSpec(
            ...
            details="Здесь находятся быстрые действия, сводка проверки и видимое состояние опорного прогона, оптимизации и диагностики.",
        ...
        DesktopWorkspaceSpec(
            ...
            route_order=90,
            kind="main",
            summary="Проверка, архив диагностики, отправка результатов и объяснимая диагностика.",
            ...
            details="Главная кнопка, быстрый поиск и карточка панели проекта обязаны вести в диагностику без обходных путей; отдельное окно остаётся инструментом восстановления.",
            units_policy="Диагностические поля показывают источник, время и тип материала, а не инженерные единицы.",
            ...
            search_aliases=_aliases_for_capabilities(("tools.diagnostics_and_bundle",), "диагностика", "архив диагностики", "проверка"),
...
def build_shell_commands() -> tuple[DesktopShellCommandSpec, ...]:
    commands = (
        ...
        DesktopShellCommandSpec("workspace.ring_editor.open", "Показать редактор циклического сценария", "Перейти к сценарию, дороге, сегментам и производным файлам.", "ring_editor", "open_workspace", "Окна -> Редактор циклического сценария", target_workspace_id="ring_editor", help_topic_id="ring_editor"),
        ...
        DesktopShellCommandSpec("workspace.animation.open", "Показать анимацию", "Перейти к аниматору, мнемосхеме и честным визуальным режимам.", "animation", "open_workspace", "Окна -> Анимация", target_workspace_id="animation", capability_ids=("visualization.animator_and_mnemo",), help_topic_id="animation"),
        ...
        DesktopShellCommandSpec("diagnostics.legacy_center.open", "Открыть диагностику отдельным окном", "Запустить диагностику отдельным окном.", "diagnostics", "launch_module", "Окна -> Диагностика -> Отдельное окно", module="pneumo_solver_ui.tools.desktop_diagnostics_center", capability_ids=("tools.diagnostics_and_bundle",), launch_surface="legacy_bridge", help_topic_id="diagnostics", search_aliases=("диагностика отдельным окном", "проверка отдельным окном"), status_label="Отдельное окно"),
