...
def build_shell_workspaces() -> tuple[DesktopWorkspaceSpec, ...]:
    workspaces = (
        ...
        DesktopWorkspaceSpec(
            ...
            quick_action_ids=("test.center.open", "workspace.baseline_run.open", "workspace.ring_editor.open"),
        ),
        DesktopWorkspaceSpec(
        ...
        details="Рабочее место обязано показывать цели расчёта, обязательные ограничения и происхождение опорного прогона рядом со входом в оптимизацию.",
            ...
            quick_action_ids=("workspace.baseline_run.open", "optimization.center.open", "optimization.primary_launch.execute", "optimization.primary_launch.soft_stop", "workspace.results_analysis.open", "workspace.diagnostics.open"),
        ),
        DesktopWorkspaceSpec(
            ...
            quick_action_ids=("results.center.open", "results.compare.prepare", "results.compare.open", "results.evidence.prepare", "results.animation.prepare", "workspace.animation.open", "workspace.diagnostics.open"),
        ),
        DesktopWorkspaceSpec(
...
def build_shell_commands() -> tuple[DesktopShellCommandSpec, ...]:
    commands = (
        ...
        DesktopShellCommandSpec("results.compare.open", "Показать сравнение", "Показать сравнение по текущему результатному контексту внутри рабочего шага анализа.", "results_analysis", "hosted_action", "Окна -> Анализ результатов -> Показать сравнение", capability_ids=("results.compare_and_review",), launch_surface="workspace", status_label="Сравнение результатов", help_topic_id="results_analysis", automation_id="RS-BTN-OPEN-COMPARE", search_aliases=("сравнение", "сравнение прогонов", "показать сравнение")),
