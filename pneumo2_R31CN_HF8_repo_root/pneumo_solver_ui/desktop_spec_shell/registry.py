...
def _migration_aliases_for_workspace(workspace_id: str) -> tuple[str, ...]:
    ...
    values: list[str] = []
    for code in WORKSPACE_CODE_BY_ID.get(workspace_id, ()):
...
def build_shell_workspaces() -> tuple[DesktopWorkspaceSpec, ...]:
    workspaces = (
        ...
        DesktopWorkspaceSpec(
            ...
            search_aliases=("базовый прогон", "baseline", "Baseline Center", "HO-006", "active_baseline_contract", "baseline history", "run setup"),
...
def build_shell_commands() -> tuple[DesktopShellCommandSpec, ...]:
    commands = (
        ...
        DesktopShellCommandSpec("ring.editor.open", "Открыть редактор кольца", "Запустить существующий ring editor как честный bridge из нового shell.", "ring_editor", "launch_module", "Рабочие пространства -> Сценарии и редактор кольца -> Редактор кольца", module="pneumo_solver_ui.tools.desktop_ring_scenario_editor", help_topic_id="ring_editor", search_aliases=("редактор кольца", "сценарии", "генерация дороги")),
        ...
        DesktopShellCommandSpec("test.center.open", "Открыть центр испытаний", "Запустить существующий test center для validation, precheck, run orchestration и HO-005 suite handoff.", "test_matrix", "launch_module", "Рабочие пространства -> Набор испытаний -> Центр испытаний", module="pneumo_solver_ui.tools.test_center_gui", capability_ids=("calculation.validation_and_prechecks",), help_topic_id="test_matrix", search_aliases=("validated suite", "validated_suite_snapshot", "suite_snapshot_hash", "HO-005", "заморозить HO-005")),
        DesktopShellCommandSpec("workspace.baseline_run.open", "Открыть workspace \"Базовый прогон\"", "Перейти к baseline provenance, baseline history и handoff в optimization.", "baseline_run", "open_workspace", "Рабочие пространства -> Базовый прогон", target_workspace_id="baseline_run", help_topic_id="baseline_run", search_aliases=("baseline", "Baseline Center", "базовый прогон", "HO-006", "active_baseline_contract", "baseline history", "run setup")),
        DesktopShellCommandSpec("baseline.center.open", "Открыть Baseline Center", "Перейти к hosted WS-BASELINE surface: HO-005, active_baseline_contract, HO-006, history и review/adopt/restore policy.", "baseline_run", "open_workspace", "Рабочие пространства -> Базовый прогон -> Baseline Center", target_workspace_id="baseline_run", status_label="Hosted workspace", help_topic_id="baseline_run", search_aliases=("Baseline Center", "HO-006 active_baseline_contract", "active_baseline_contract", "HO-006", "review adopt restore", "baseline history", "запустить baseline", "базовый запуск")),
        DesktopShellCommandSpec("baseline.review", "Review selected baseline", "Read-only просмотр selected active/history candidate, mismatch matrix и HO-006 policy без изменения active_baseline_contract.", "baseline_run", "hosted_action", "Рабочие пространства -> Базовый прогон -> Review baseline", status_label="Read-only", help_topic_id="baseline_run", automation_id="BL-BTN-REVIEW", search_aliases=("baseline review", "review baseline", "HO-006 review", "active_baseline_contract review", "baseline mismatch matrix")),
        DesktopShellCommandSpec("baseline.adopt", "Adopt candidate baseline", "Явно принять current/valid candidate как новый active_baseline_contract; silent rebinding запрещён.", "baseline_run", "hosted_action", "Рабочие пространства -> Базовый прогон -> Adopt baseline", status_label="Explicit action", help_topic_id="baseline_run", automation_id="BL-BTN-ADOPT", search_aliases=("baseline adopt", "adopt baseline", "HO-006 adopt", "active_baseline_contract adopt", "explicit baseline adopt")),
        DesktopShellCommandSpec("baseline.restore", "Restore historical baseline", "Явно восстановить historical baseline как active; mismatch по suite_snapshot_hash, inputs_snapshot_hash или policy_mode требует предупреждения и подтверждения.", "baseline_run", "hosted_action", "Рабочие пространства -> Базовый прогон -> Restore baseline", status_label="Explicit action", help_topic_id="baseline_run", automation_id="BL-BTN-RESTORE", search_aliases=("baseline restore", "restore baseline", "HO-006 restore", "historical baseline restore", "suite_snapshot_hash inputs_snapshot_hash policy_mode")),
        DesktopShellCommandSpec("baseline.legacy_launch.open", "Открыть legacy baseline launch surface", "Запустить существующий test center bridge для baseline-aware launch surface.", "baseline_run", "launch_module", "Рабочие пространства -> Базовый прогон -> Legacy launch surface", module="pneumo_solver_ui.tools.test_center_gui", status_label="Legacy bridge", help_topic_id="baseline_run", search_aliases=("legacy baseline", "test center baseline", "baseline launch")),
