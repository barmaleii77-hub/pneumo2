...
ROUTE_QUICK_ACTIONS_BY_WORKSPACE: dict[str, tuple[str, ...]] = {
    ...
    "baseline_run": ("baseline.run_setup.open", "baseline.run.execute", "baseline.review", "baseline.adopt", "baseline.restore", "workspace.optimization.open", "workspace.results_analysis.open"),
    "optimization": ("workspace.baseline_run.open", "optimization.center.open", "optimization.primary_launch.execute", "optimization.primary_launch.soft_stop", "workspace.results_analysis.open", "workspace.diagnostics.open"),
    "results_analysis": ("results.center.open", "results.compare.prepare", "results.compare.open", "results.animation.prepare", "results.evidence.prepare", "workspace.animation.open", "workspace.diagnostics.open"),
...
def build_shell_workspaces() -> tuple[DesktopWorkspaceSpec, ...]:
    workspaces = (
        ...
        details="Рабочий шаг показывает условия запуска и открывает существующие окна без технических меток.",
            ...
            quick_action_ids=("baseline.run_setup.open", "baseline.run.execute", "baseline.review", "baseline.adopt", "baseline.restore", "workspace.optimization.open", "workspace.results_analysis.open"),
        ),
        DesktopWorkspaceSpec(
        ...
        details="Рабочее место открывает анализ результатов и окно сравнения как согласованный рабочий шаг анализа.",
            ...
            quick_action_ids=("results.center.open", "analysis.engineering.open", "results.compare.open", "results.animation.prepare"),
        ),
        DesktopWorkspaceSpec(
...
def build_shell_commands() -> tuple[DesktopShellCommandSpec, ...]:
    commands = (
        ...
        DesktopShellCommandSpec("test.legacy_center.open", "Расширенная настройка набора", "Открыть прежний центр набора испытаний для специальных сценариев и детальной настройки.", "test_matrix", "launch_module", "Окна -> Набор испытаний -> Расширенная настройка", module="pneumo_solver_ui.tools.test_center_gui", capability_ids=("calculation.validation_and_prechecks",), launch_surface="legacy_bridge", status_label="Рабочее окно", help_topic_id="test_matrix", search_aliases=("расширенная настройка набора", "подробная настройка испытаний", "центр испытаний")),
        ...
        DesktopShellCommandSpec("baseline.run.open_log", "Открыть журнал базового прогона", "Открыть журнал последнего подготовленного или выполненного базового прогона.", "baseline_run", "hosted_action", "Окна -> Базовый прогон -> Открыть журнал", capability_ids=("calculation.run_setup", "calculation.baseline_run", "calculation.launch"), launch_surface="workspace", status_label="Журнал запуска", help_topic_id="baseline_run", automation_id="BL-BTN-RUN-OPEN-LOG", search_aliases=("журнал базового прогона", "лог запуска", "открыть лог расчёта")),
        DesktopShellCommandSpec("baseline.run.open_result", "Открыть папку результата", "Открыть папку результата последнего подготовленного или выполненного базового прогона.", "baseline_run", "hosted_action", "Окна -> Базовый прогон -> Открыть папку результата", capability_ids=("calculation.run_setup", "calculation.baseline_run", "calculation.launch"), launch_surface="workspace", status_label="Папка результата", help_topic_id="baseline_run", automation_id="BL-BTN-RUN-OPEN-RESULT", search_aliases=("папка результата", "результат базового прогона", "артефакты расчёта")),
        DesktopShellCommandSpec("baseline.legacy_run_setup.open", "Расширенный центр запуска", "Открыть расширенную настройку расчёта и запусков для детальной работы.", "baseline_run", "launch_module", "Окна -> Базовый прогон -> Расширенный центр запуска", module="pneumo_solver_ui.tools.desktop_run_setup_center", capability_ids=("calculation.run_setup", "calculation.baseline_run", "calculation.launch"), launch_surface="legacy_bridge", status_label="Рабочее окно", help_topic_id="baseline_run", search_aliases=("расширенная настройка расчёта", "центр запуска", "подробная настройка расчёта")),
        ...
        DesktopShellCommandSpec("optimization.primary_launch.open_log", "Открыть журнал оптимизации", "Открыть журнал последнего активного или подготовленного запуска оптимизации.", "optimization", "hosted_action", "Окна -> Оптимизация -> Открыть журнал", capability_ids=("optimization.orchestration_and_databases",), launch_surface="workspace", status_label="Журнал запуска", help_topic_id="optimization", automation_id="OP-BTN-OPEN-LOG", search_aliases=("журнал оптимизации", "лог StageRunner", "открыть лог оптимизации")),
        DesktopShellCommandSpec("optimization.primary_launch.open_run_dir", "Открыть папку запуска оптимизации", "Открыть папку артефактов последнего активного или подготовленного запуска оптимизации.", "optimization", "hosted_action", "Окна -> Оптимизация -> Открыть папку запуска", capability_ids=("optimization.orchestration_and_databases",), launch_surface="workspace", status_label="Папка запуска", help_topic_id="optimization", automation_id="OP-BTN-OPEN-RUN-DIR", search_aliases=("папка оптимизации", "артефакты StageRunner", "открыть папку запуска")),
        DesktopShellCommandSpec("optimization.legacy_center.open", "Расширенная настройка оптимизации", "Открыть подробную настройку оптимизации для специальных сценариев.", "optimization", "launch_module", "Окна -> Оптимизация -> Расширенная настройка", module="pneumo_solver_ui.tools.desktop_optimizer_center", capability_ids=("optimization.orchestration_and_databases",), launch_surface="legacy_bridge", status_label="Рабочее окно", help_topic_id="optimization", search_aliases=("подробная настройка оптимизации", "расширенная оптимизация", "координация расчётов")),
        ...
        DesktopShellCommandSpec("results.legacy_center.open", "Расширенный анализ результатов", "Открыть подробный анализ результатов для специальных сценариев.", "results_analysis", "launch_module", "Окна -> Анализ результатов -> Расширенный анализ", module="pneumo_solver_ui.tools.desktop_results_center", capability_ids=("results.compare_and_review", "analysis.influence_and_exploration"), launch_surface="legacy_bridge", status_label="Рабочее окно", help_topic_id="results_analysis", search_aliases=("подробный анализ результатов", "расширенный анализ", "графики результатов")),
        DesktopShellCommandSpec("results.compare.open", "Открыть сравнение", "Открыть сравнение по текущему результатному контексту из рабочего шага анализа.", "results_analysis", "hosted_action", "Окна -> Анализ результатов -> Открыть сравнение", capability_ids=("results.compare_and_review",), launch_surface="workspace", status_label="Сравнение результатов", help_topic_id="results_analysis", automation_id="RS-BTN-OPEN-COMPARE", search_aliases=("сравнение", "сравнение прогонов", "открыть сравнение")),
        DesktopShellCommandSpec("results.legacy_compare.open", "Расширенный просмотр сравнения", "Открыть расширенный просмотр сравнения напрямую, если нужен аварийный или специальный запуск вне текущего контекста анализа.", "results_analysis", "launch_module", "Окна -> Анализ результатов -> Расширенный просмотр сравнения", module="pneumo_solver_ui.qt_compare_viewer", capability_ids=("results.compare_and_review",), launch_surface="external_window", help_topic_id="results_analysis", search_aliases=("расширенное сравнение", "просмотр сравнения", "аварийное сравнение")),
        ...
        DesktopShellCommandSpec("animation.animator.launch", "Проверить движение", "Запустить подробную проверку движения из текущего контекста рабочего шага.", "animation", "hosted_action", "Окна -> Анимация -> Проверить движение", capability_ids=("visualization.animator_and_mnemo",), launch_surface="workspace", status_label="Графическая проверка", help_topic_id="animation", automation_id="AM-DETACH", search_aliases=("проверить движение", "графическая проверка", "анализ движения")),
        ...
        DesktopShellCommandSpec("animation.legacy_animator.open", "Расширенный просмотр анимации", "Открыть подробный просмотр анимации для графической проверки.", "animation", "launch_module", "Окна -> Анимация -> Расширенный просмотр анимации", module="pneumo_solver_ui.desktop_animator.app", capability_ids=("visualization.animator_and_mnemo",), launch_surface="external_window", help_topic_id="animation", search_aliases=("расширенная анимация", "подробная анимация", "трёхмерный вид")),
        DesktopShellCommandSpec("animation.legacy_mnemo.open", "Расширенный просмотр мнемосхемы", "Открыть подробный просмотр мнемосхемы для графической проверки.", "animation", "launch_module", "Окна -> Анимация -> Расширенный просмотр мнемосхемы", module="pneumo_solver_ui.desktop_mnemo.main", capability_ids=("visualization.animator_and_mnemo",), launch_surface="external_window", help_topic_id="animation", search_aliases=("расширенная мнемосхема", "подробная мнемосхема", "пневмосхема")),
