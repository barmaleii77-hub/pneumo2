...
def build_shell_workspaces() -> tuple[DesktopWorkspaceSpec, ...]:
    workspaces = (
        ...
        DesktopWorkspaceSpec(
            ...
            route_order=40,
            kind="main",
            summary="Master-copy набора испытаний, стадий, приоритетов и связей со сценариями.",
            ...
            hard_gate="Без явного test matrix невозможно честно запускать baseline и optimization.",
            details="На wave 1 используется bridge к текущему test center, но shell держит отдельный workspace.",
            ...
            search_aliases=_aliases_for_capabilities(("calculation.validation_and_prechecks",), "набор испытаний", "испытания", "test matrix"),
...
def build_shell_commands() -> tuple[DesktopShellCommandSpec, ...]:
    commands = (
        ...
        DesktopShellCommandSpec("workspace.test_matrix.open", "Открыть workspace \"Набор испытаний\"", "Перейти к матрице испытаний, stage-логике и validation/precheck lane.", "test_matrix", "open_workspace", "Рабочие пространства -> Набор испытаний", target_workspace_id="test_matrix", capability_ids=("calculation.validation_and_prechecks",), help_topic_id="test_matrix"),
        DesktopShellCommandSpec("test.center.open", "Открыть центр испытаний", "Запустить существующий test center для validation, precheck и run orchestration.", "test_matrix", "launch_module", "Рабочие пространства -> Набор испытаний -> Центр испытаний", module="pneumo_solver_ui.tools.test_center_gui", capability_ids=("calculation.validation_and_prechecks",), help_topic_id="test_matrix"),
