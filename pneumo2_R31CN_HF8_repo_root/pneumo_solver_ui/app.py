...
from pneumo_solver_ui.ui_workflow_shell_helpers import (
    render_home_workflow_header,
    render_workflow_mode_and_section,
    workflow_visibility,
)
...
from pneumo_solver_ui.ui_suite_editor_section_helpers import (
    render_app_suite_editor_section,
)
...
with st.sidebar:
    ...
    ui_compact = st.checkbox(
        ...
        help=(
            "Делает рабочую область компактнее и убирает лишние визуальные отступы. "
            "Удобно для небольшого экрана и для пошаговой работы."
        ),
    )
    st.session_state["ui_compact"] = ui_compact

    st.checkbox(
        "При запуске Play упрощать Plotly-графики",
        value=st.session_state.get("skip_heavy_on_play", True),
        key="skip_heavy_on_play",
        help=(
            "Во время воспроизведения тяжелые Plotly-графики временно отключаются в фоне, "
            "чтобы анимация шла плавнее и стабильнее на слабых машинах."
        ),
    )
...
render_home_workflow_header(st)
...
_ui_mode, ui_section = render_workflow_mode_and_section(choose_fn=_seg_or_radio)
_workflow = workflow_visibility(ui_section)
SHOW_MODEL = bool(_workflow["show_model"])
SHOW_PARAMS = bool(_workflow["show_params"])
SHOW_TESTS = bool(_workflow["show_tests"])
SHOW_RUN = bool(_workflow["show_run"])
SHOW_RESULTS = bool(_workflow["show_results"])
SHOW_TOOLS = bool(_workflow["show_tools"])
...
# -------------------------------
if SHOW_MODEL:
    st.subheader("1. Модель: файлы проекта")
    st.caption(
        "Сначала выберите файл модели и файл оптимизатора. Это помогает держать "
        "под рукой только актуальные файлы проекта и не путаться между версиями."
    )
    ...
    st.info(f"Текущие файлы: модель {Path(model_path).name}, оптимизатор {Path(worker_path).name}")
...
if SHOW_TESTS:
    df_suite_edit = render_app_suite_editor_section(
        st,
        ...
        normalize_suite_df_for_editor_fn=_normalize_suite_df_for_editor,
        load_default_suite_disabled_fn=load_default_suite_disabled,
        first_suite_selected_index_fn=first_suite_selected_index,
    )
...
if SHOW_RESULTS or SHOW_TOOLS:
    ...
    if st.session_state.baseline_df is None:
        ...
        if not avail:
            ...
            with colG3:
                want_full = st.checkbox("Полный лог record_full (потоки и состояния)", value=True, key="detail_want_full")
            ...
            with st.expander("Desktop Animator (Windows) — информативная анимация", expanded=True):
                ...
                try:
                    if _npz_p.exists():
                        _st.append(f"NPZ: готов {_npz_p.name} ({_npz_p.stat().st_size/1024:.1f} KB)")
                    else:
                        _st.append(f"NPZ: нет {_npz_p.name}")
                    if _ptr_p.exists():
                        _st.append(f"PTR: готов {_ptr_p.name}")
                    else:
                        _st.append(f"PTR: нет {_ptr_p.name}")
                except Exception:
                    pass
                if _st:
                    st.info("\n".join(_st))
                ...
                with colA1:
                    ...
                    if st.button("Запустить Mnemo (--follow)", key="launch_desktop_mnemo"):
                        try:
                            cmd = [sys.executable, "-m", "pneumo_solver_ui.desktop_mnemo.main", "--follow", "--theme", "dark"]
                            start_worker(cmd, cwd=HERE)
                            st.success("Desktop Mnemo запущен.")
                        except Exception as e:
                            st.error(f"Не удалось запустить Desktop Mnemo: {e}")
                with colA2:
                ...
                st.caption(
                    ...
                    "Если включён авто‑экспорт — Desktop Animator и Desktop Mnemo обновится автоматически."
                )
