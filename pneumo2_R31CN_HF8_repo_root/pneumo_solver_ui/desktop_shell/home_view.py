...
WORKFLOW_HINTS: dict[str, str] = {
    ...
    "desktop_diagnostics_center": "Соберите архив для отправки и подготовьте материалы после проверки результата.",
    "autotest_gui": "Запускайте автотест напрямую, когда нужен отдельный контур прогона без лишних экранов.",
    "full_diagnostics_gui": "Запустите расширенную проверку проекта перед разбором проблем или отправкой архива.",
...
V10_FIRST_PATH_TEXT = (
    ...
    "оптимизация; анализ; анимация; проверка и отправка."
)
...
CARD_HINTS: dict[str, str] = {
    "desktop_diagnostics_center": (
        "Основной порядок проверки состояния проекта и подготовки архива для отправки. "
        "Начинайте отсюда, если нужно разобраться с ошибкой или передать материалы."
    ),
    "full_diagnostics_gui": (
        "Расширенная проверка для технического разбора. Обычно начинайте с проверки и отправки."
    ),
    "send_results_gui": (
        "Дополнительное окно для подготовки материалов к отправке после проверки проекта и результатов."
    ),
    "compare_viewer": (
...
ACTION_BUTTON_TEXTS: dict[str, str] = {
    ...
    "desktop_diagnostics_center": "Собрать архив для отправки",
...
def build_shell_home_view(
    ...
    *,
    ...
    _build_group_box(
        ...
        diagnostics_specs,
        open_tool,
        intro="Один понятный порядок работы: проверить проект и подготовить архив для отправки без поиска по разным окнам.",
        columnspan=2,
        primary_key="desktop_diagnostics_center",
        primary_button_text="Собрать архив для отправки",
