...
WORKFLOW_HINTS: dict[str, str] = {
    ...
    "desktop_diagnostics_center": "Соберите диагностику и сохраните архив проекта после проверки результата.",
...
CARD_HINTS: dict[str, str] = {
    "desktop_diagnostics_center": (
        "Основной порядок диагностики состояния проекта и сохранения архива проекта. "
        "Начинайте отсюда, если нужно разобраться с ошибкой или сохранить материалы."
    ),
    "full_diagnostics_gui": (
        "Расширенная проверка для технического разбора. Обычно начинайте с диагностики."
    ),
    "send_results_gui": (
        "Дополнительное окно для копирования сохранённого архива после диагностики и проверки результатов."
    ),
    "compare_viewer": (
...
def build_shell_home_view(
    ...
    *,
    ...
    _build_group_box(
        ...
        diagnostics_specs,
        open_tool,
        intro="Один понятный порядок работы: собрать диагностику и сохранить архив проекта без поиска по разным окнам.",
        columnspan=2,
