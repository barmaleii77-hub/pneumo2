...
WORKFLOW_HINTS: dict[str, str] = {
    ...
    "desktop_diagnostics_center": "Проверьте проект и сохраните архив проекта после проверки результата.",
    "autotest_gui": "Запускайте автотест напрямую, когда нужен отдельный контур прогона без лишних экранов.",
    "full_diagnostics_gui": "Запустите расширенную проверку проекта перед разбором проблем или сохранением архива.",
    "send_results_gui": "Скопируйте сохранённый архив проекта вручную в отдельном окне.",
}

V10_FIRST_PATH_TEXT = (
    "Что делать сначала: исходные данные; сценарии; набор испытаний; базовый прогон; "
    "оптимизация; анализ; анимация; проверка проекта."
)
...
CARD_HINTS: dict[str, str] = {
    "desktop_diagnostics_center": (
        "Основной порядок проверки состояния проекта и сохранения архива проекта. "
        "Начинайте отсюда, если нужно разобраться с ошибкой или сохранить материалы."
    ),
    "full_diagnostics_gui": (
        "Расширенная проверка для технического разбора. Обычно начинайте с проверки проекта."
    ),
    "send_results_gui": (
        "Дополнительное окно для копирования сохранённого архива после проверки проекта и результатов."
    ),
    "compare_viewer": (
...
ACTION_BUTTON_TEXTS: dict[str, str] = {
    ...
    "desktop_diagnostics_center": "Сохранить архив проекта",
    "full_diagnostics_gui": "Запустить расширенную проверку",
    "send_results_gui": "Скопировать архив",
    "autotest_gui": "Запустить автотесты",
    "compare_viewer": "Подробное сравнение",
    "desktop_mnemo": "Открыть пневмосхему",
...
def build_shell_home_view(
    ...
    *,
    ...
    ttk.Label(
        parent,
        text=(
            "Классическое рабочее место показывает один порядок первого запуска. "
    ...
    _build_group_box(
        ...
        diagnostics_specs,
        open_tool,
        intro="Один понятный порядок работы: проверить проект и сохранить архив проекта без поиска по разным окнам.",
        columnspan=2,
        primary_key="desktop_diagnostics_center",
        primary_button_text="Сохранить архив проекта",
        secondary_title="Дополнительные действия после проверки",
    )
    _build_group_box(
        cards,
        1,
