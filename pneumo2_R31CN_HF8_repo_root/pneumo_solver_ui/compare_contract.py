...
def _short(value: Any, n: int = 12) -> str:
    ...
    return text[: max(1, int(n))] if text else "-"
...
def build_compare_contract(
    ...
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "run_refs": refs,
        "baseline_ref": _jsonable(dict(refs[0].get("baseline_ref") or {})) if refs else {},
        "objective_ref": _jsonable(dict(refs[0].get("objective_ref") or {})) if refs else {},
...
def format_compare_mismatch_banner(summary: Mapping[str, Any] | None) -> str:
    ...
    if banner_id == "BANNER-HIST-002":
        ...
        return (
            "Current and historical context differ: "
            f"{suffix}. Compare uses pinned refs and does not mutate optimizer history."
        )
    if banner_id == "BANNER-HIST-003":
        suffix = ", ".join(dims[:6]) if dims else "artifact refs"
        return f"Compare contract is incomplete: {suffix}. Silent fallback to the current project is blocked."
    if banner_id == "BANNER-HIST-001":
        return "Historical compare context is open and refs match the selected context."
    return ""
...
def format_compare_contract_summary(contract: Mapping[str, Any] | None) -> str:
    ...
    objective_hashes = sorted(
        {_short(ref.get("objective_contract_hash")) for ref in refs if ref.get("objective_contract_hash")}
    )
    baseline_hashes = sorted(
        {_short(ref.get("active_baseline_hash")) for ref in refs if ref.get("active_baseline_hash")}
    )
